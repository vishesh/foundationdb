	...
	loop {
		...
		for (it = data->metrics.begin(); it != data->metrics.end(); ++it) {
			...
			for (const auto& [p, m] : metrics) {
				...
				closedCounts.push_back(m.back().closedConnectionCount);
				std::sort(closedCounts.begin(), closedCounts.end());
			}

			int closedCount = closedCounts[closedCounts.size() / 2];
			...
			TraceEvent("FailureMonitorCollectedStatus")
				...
				.detail("ClosedCount", closedCount);
